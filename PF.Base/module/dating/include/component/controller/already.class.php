 ...
 */
class Dating_Component_Controller_Already extends Phpfox_Component
{
	/**
	 * Controller
	 */
	public function process()
	{
        ...
        $this->template()->setBreadCrumb(_p("Skipped Users"), $this->url()->makeUrl("dating.already"));
        ...
        $this->search()->set(array(
                ...
                'search_tool' => array(
                    'table_alias' => 'f',
                    'search' => array(
                        'action' => $this->url()->makeUrl('dating.already'),
                        'default_value' => _p('Search users'),
                        'name' => 'search',
                        'field' => 'u.full_name'
                    ),
                    'sort' => array(
                        'latest' => array('f.action_id', _p('latest')),
                    ),
                    'show' => array(12, 16, 24)
                )
            )
        ...
        $aBrowseParams = array(
            ...
            'alias' => 'f',
            'field' => 'action_id',
            'table' => Phpfox::getT('dating_actions')
        );
        $this->search()->setCondition(" AND f.action='next' AND f.owner_id=".Phpfox::getUserId());

 ...
 */
class Dating_Component_Controller_Wholike extends Phpfox_Component
{
	/**
	 * Controller
	 */
	public function process()
	{
        ...
        //HIDE NOTIFICATIOIN
        Dating_Service_Dating::instance()->hideLikes();

        $this->template()->setTitle(_p('Dating'));
        $this->template()->setBreadCrumb(_p('Dating'), $this->url()->makeUrl("dating"));
        $this->template()->setBreadCrumb(_p("Who liked Me"), $this->url()->makeUrl("dating.wholike"));
        ...
        $this->search()->set(array(
                ...
                'search_tool' => array(
                    'table_alias' => 'f',
                    'search' => array(
                        'action' => $this->url()->makeUrl('dating.wholike'),
                        'default_value' => _p('Search users'),
                        'name' => 'search',
                        'field' => 'u.full_name'
                    ),
                    'sort' => array(
                        'latest' => array('f.like_id', _p('latest')),
                    ),
                    'show' => array(12, 16, 24)
                )
            )
        ...
        $aBrowseParams = array(
            ...
            'alias' => 'f',
            'field' => 'like_id',
            'table' => Phpfox::getT('dating_likes')
        );
        $this->search()->setCondition(" AND f.is_own=1 AND f.mutual=0 AND f.user_id=".Phpfox::getUserId());
        ...
        if (!empty($aUsers)) {
            foreach($aUsers as &$aUser) {
                $aUser["real"] = User_Service_User::instance()->get($aUser["owner_id"], true);
            }
        }

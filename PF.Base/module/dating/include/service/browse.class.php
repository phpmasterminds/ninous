 ...
 */
class Dating_Service_Browse extends Phpfox_Service
{
    ...
    public function isDating($bGender)
    {
        $this->_isDating = $bGender;
	...
	public function get()
	{
		...
		if (!defined('PHPFOX_IS_ADMIN_SEARCH'))
		{
			...
			$aHiddenFromBrowse = User_Service_Group_Setting_Setting::instance()->getUserGroupsBySetting('user.hide_from_browse');
		}
		...
		if (!isset($bNoMatches))
		{
            ...
            if ($this->_isDating) {
                $this->database()->join(Phpfox::getT("dating_values"), "d", "u.user_id = d.user_id");
            }
		...
		if ($iCnt > 0)
		{
            ...
            if ($this->_isDating) {
                $this->database()->leftJoin(Phpfox::getT("dating_values"), "d", "u.user_id = d.user_id");
            }
			...
			if ($this->_bExtend)
			{
				foreach ($aReturnUsers as $iKey => $aUser)
				{
					...
					$aBirthDay = User_Service_User::instance()->getAgeArray($aUser['birthday']);

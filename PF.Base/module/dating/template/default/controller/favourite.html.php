...
{if ($aUsers)}
{if !PHPFOX_IS_AJAX}
<div class="wrapper-items item-container user-listing" id="collection-users" style="margin-top:20px;">
    {/if}
    {foreach from=$aUsers name=friend item=aUser}
            ...
            <div class="item-inner">
                ...
                <div class="user-info">
                    <div class="user-title">
                        <a href="{url link='dating'}?user={$aUser.user_name}">{$aUser.full_name}</a>
                    </div>
                    {if !empty($aUser.rating_html)}
                    <div class='browse_rating' style="margin-bottom: 5px;">{$aUser.rating_html}
                        {if $aUser.recommend}<span style="font-size:11px;padding-left:3px;">({$aUser.recommend} {_p var='reviews'})</span>{/if}
                    </div>
                    {/if}

                    {module name='user.friendship' friend_user_id=$aUser.user_id type='icon' extra_info=false no_button=true mutual_list=true}
...
<div class="extra_info">
    {_p var='No favourites users found.'}
</div>
{/if}